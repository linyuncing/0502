...
## 第六條指令
```tex
取消graphics每個單位上面的圖，改為寬為18的方框，中間有個5的圓，圓的顏色為ffe5d9，方框的顏色採用capture的相對位置的顏色
```
更新點：
方框：
使用 graphics.rect(x + 1, y + 1, 18, 18) 繪製寬為 18 的方框。
方框的顏色來自 capture.get(x, y)。
圓：
使用 graphics.ellipse(x + 10, y + 10, 5, 5) 在方框中間繪製直徑為 5 的圓。
圓的顏色固定為 #ffe5d9。
雙層迴圈：
每隔 20 單位繪製一個方框和圓。
### 程式碼
